...
- (void)viewDidLoad
{
  ...
  [self.topViewSnapshot addGestureRecognizer:self.resetTapGesture];
    ...
    //Assigning points to zero

    panAmount = 0.0 ;
    newCenterPosition = 0.0;
}
...
- (void)updateTopViewHorizontalCenterWithRecognizer:(UIPanGestureRecognizer *)recognizer
{
  ...
  if (recognizer.state == UIGestureRecognizerStateBegan) {
    ...
    self.initialHoizontalCenter = self.topView.center.x;
     ...
     panAmount = self.initialTouchPositionX - currentTouchPositionX;
     newCenterPosition = self.initialHoizontalCenter - panAmount;
    ...
    //CGFloat currentVelocityX     = currentVelocityPoint.x;
      ...
      // code for stopping the view at any point of time.
//      NSLog(@"Pan amount is %f",panAmount);
//      NSLog(@"new point is %f",newCenterPosition);

      if ([self underLeftShowing] && newCenterPosition >300) {
